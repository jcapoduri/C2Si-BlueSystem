...
void wUserLogBrowser::refreshEmployeeLog()
{
    ...
    _query = QString("SELECT DATE_FORMAT(initstamp, '%d-%m') as dia, DATE_FORMAT(initstamp, '%H:%i:%s') as i, DATE_FORMAT(endstamp, '%H:%i:%s') as e FROM userstamp WHERE %1").arg(_where);
    QSqlQuery q = nd::connection::instance()->select(_query);
